(() => {
  ...
  function lastGameDate(game) {
    const raw = game?.completedAt || game?.startedAt || '';
    if (!raw) return '—';
  ...
  function lastGameTotal(game, side) {
    const regulation = (game?.innings?.[side] || []).reduce((sum, value) => sum + (Number(value) || 0), 0);
    return (game?.tieBreaks || []).reduce((sum, item) => sum + (Number(item?.[side]) || 0), regulation);
  }

  function lastGameScore(value) {
    return value === '' || value === null || value === undefined ? '—' : String(value);
  }

  function openLastGameModal() {
    const game = normalizeGame(state.lastGame) || rememberedLastGame();
    const modal = document.getElementById('liveScoreLastGameModal');
    if (!game || !modal) return;
    const setText = (id, value) => { const el = document.getElementById(id); if (el) el.textContent = value || '—'; };
    setText('liveScoreLastGameTournament', game.tournament);
    setText('liveScoreLastGameDate', lastGameDate(game));
    setText('liveScoreLastGameGround', game.ground);
    setText('liveScoreLastGameTime', game.startTime);
    setText('liveScoreLastGameGrade', game.grade ? `${game.grade}年生` : '—');

    const board = document.getElementById('liveScoreLastGameBoard');
    if (board) {
      const tieBreaks = Array.isArray(game.tieBreaks) ? game.tieBreaks : [];
      const heads = ['1','2','3','4','5','6','7'];
      if (tieBreaks.length) heads.push('TB');
      const ourScores = game.innings.ours.map(lastGameScore);
      const oppScores = game.innings.opponent.map(lastGameScore);
      if (tieBreaks.length) {
        ourScores.push(String(tieBreaks.reduce((sum, item) => sum + (Number(item.ours) || 0), 0)));
        oppScores.push(String(tieBreaks.reduce((sum, item) => sum + (Number(item.opponent) || 0), 0)));
      }
      const esc = value => String(value ?? '').replace(/[&<>"']/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
      board.innerHTML = `<thead><tr><th class="team">チーム名</th>${heads.map(h=>`<th>${h}</th>`).join('')}<th>計</th></tr></thead>` +
        `<tbody><tr><td class="team ours">${esc(game.ourName || '八千代')}</td>${ourScores.map(v=>`<td>${esc(v)}</td>`).join('')}<td class="total">${lastGameTotal(game,'ours')}</td></tr>` +
        `<tr><td class="team">${esc(game.opponent || '相手チーム')}</td>${oppScores.map(v=>`<td>${esc(v)}</td>`).join('')}<td class="total">${lastGameTotal(game,'opponent')}</td></tr></tbody>`;
    }
    modal.hidden = false;
  ...
  document.getElementById('liveScoreLastGameClose')?.addEventListener('click', closeLastGameModal);
  document.getElementById('liveScoreLastGameBack')?.addEventListener('click', closeLastGameModal);
  document.getElementById('liveScoreLastGameModal')?.addEventListener('click', event => {
    if (event.target?.id === 'liveScoreLastGameModal') closeLastGameModal();
  });
