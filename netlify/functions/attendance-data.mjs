...
function importMatchingDensukeData(data, html) {
  ...
  data.comments = data.comments.filter(comment => comment?.source !== "densuke");
...
export default async (request, context) => {
  ...
  try {
    if (request.method === "GET") {
      ...
      if (config.migrationEnded && data.densukeImportVersion < 2) {
        try {
          ...
          console.error("parent Densuke repair import error:", error);
        }
      }
    ...
    if (action === "comment") {
      ...
      const eventDate = /^\d{4}-\d{2}-\d{2}$/.test(String(body.eventDate || "")) ? String(body.eventDate) : "";
      const escortGrade = ['2','3'].includes(String(body.escortGrade || '')) ? String(body.escortGrade) : '';
      const upperGrade = body.upperGrade === true || !!escortGrade;
